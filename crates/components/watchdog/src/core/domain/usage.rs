...
pub fn find_current_session_window(
    ...
    Some(SessionWindow { start_epoch: session_start, end_epoch: session_end })
}
...
pub fn aggregate_by_model(records: &[UsageRecord]) -> Vec<ModelStats> {
    ...
    for r in records {
        ...
        let entry = by_model.entry(r.model.clone()).or_insert_with(|| ModelStats {
            model:                       r.model.clone(),
            turns:                       0,
            input_tokens:                0,
            output_tokens:               0,
...
mod tests {
    ...
    fn long_continuous_activity_is_one_session() {
        ...
        let r: Vec<UsageRecord> =
            (0..5).map(|i| rec("opus", 1000 + i * 1800, 1)).collect();
