...
use crate::core::application::{UsageLogReader, UsageReadError};
...
#[allow(clippy::struct_field_names, reason = "field names mirror the JSON schema verbatim")]
struct LogUsage {
    #[serde(default)]
    input_tokens:                u64,
    #[serde(default)]
...
impl ClaudeCodeLogReader {
    ...
    pub fn with_default_roots() -> Self {
        ...
        if let Ok(home) = std::env::var("HOME") {
            ...
            roots.push(PathBuf::from(&home).join(".claude-personal").join("projects"));
        }
    ...
    pub fn for_claude_dir(claude_dir: &Path) -> Self {
        Self { roots: vec![claude_dir.join("projects")] }
    }
}
...
impl UsageLogReader for ClaudeCodeLogReader {
    fn read_all(&self) -> Result<Vec<UsageRecord>, UsageReadError> {
        ...
        for root in &self.roots {
            ...
            collect_jsonl(root, &mut records)
                .map_err(|e| UsageReadError::Io(e.to_string()))?;
        }
        if !any_root_exists {
...
fn parse_jsonl_file(path: &Path, out: &mut Vec<UsageRecord>) -> std::io::Result<()> {
    ...
    for line in reader.lines() {
        ...
        let Some(timestamp) = parsed.timestamp else { continue };
        let Ok(ts) = timestamp.parse::<jiff::Timestamp>() else { continue };
