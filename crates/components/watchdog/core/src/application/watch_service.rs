//! Watchdog use case.
//!
//! Pure orchestration: every IO operation goes through a port (`Pane`, `Clock`,
//! `StopSignal`, `Presenter`). All time math lives in [`crate::domain`].
...
#[derive(Debug)]
pub struct WatchService<P, C, S, V, U> {
    pane: P,
    clock: C,
    stop: S,
    presenter: V,
...
impl<P, C, S, V, U> WatchService<P, C, S, V, U>
where
    P: Pane,
    C: Clock,
    S: StopSignal,
    ...
    U: UsageLogReader,
{
    ...
    pub fn run(&self) -> Result<RunStats, WatchError> {
        ...
        if !self.pane.exists(&self.cfg.session) {
    ...
    fn check_once(&self, state: &mut State) -> Result<(), WatchError> {
        ...
        let pane = self.pane.capture(&self.cfg.session, self.cfg.pane_lines)?;
...
mod tests {
    ...
    use super::*;
    use crate::application::ports::{IdleInfo, MockClock, MockPane, MockPresenter, MockStopSignal};
    use crate::application::usage_report::MockUsageLogReader;
...
";
    ...
    fn missing_session_at_start_is_an_error() {
        ...
        pane.expect_exists().with(eq("work")).return_const(false);
